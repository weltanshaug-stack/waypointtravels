...
DEPARTURE: the VERY LAST item of the LAST day must always be boarding the flight home from the destination's real main airport — title it like "Board flight home from Haneda Airport", set timeOfDay to the latest slot used that day, and set its imageQuery to that airport plus city (e.g. "Haneda Airport terminal Tokyo", "Charles de Gaulle Airport Paris") so the photo shows the airport or an aircraft there. No activity may come after it.
